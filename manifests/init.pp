...
class filebeat (
  ...
  Optional[String]  $conf_template        = $filebeat::params::conf_template,
  ...
  Boolean $use_generic_template = $filebeat::params::use_generic_template,
  ...
  $real_conf_template = $use_generic_template ? {
    true => "${module_name}/pure_hash.erb",
  ...
  if $package_ensure != 'absent' {
    ...
    if !empty($processors) {
      create_resources('filebeat::processor', $processors)
    }
  }
}
